...
$sa(document).on(
    "click", ".sa_default_login_form,.sa_default_signup_form",
    function() {
        ...
        parentForm.next().find(".sa-lwo-form").fadeIn(
            ...
            function() {
                ...
                if (isInputVisibleToUser(mob_field)) {
                    $sa('html').animate({
                        scrollTop: mob_field.offset().top - 100
                    }, 500, function() {
                        mob_field.focus();
                    });
                }
...
$sa(document).on(
    "keyup", ".smsalertModal .otp-number",
    function(e) {
        var otp_length = $sa('#smsalert_customer_validation_otp_token').attr('data-max');
        var txtData = [];
...
function showSAModal(o, cur_btn, phone_selector, submit_selector, prev_btn_text, success_cb, otp_resend_timer) {
        ...
        currentModel.find(".otp_input,.otp-number").off().on(
            ...
            function(event) {
                if (event.which === 13) {
                    ...
                    validateOtp({
                        data: {
                            btn_class: submit_selector
                        }
                    });
                }
            }
        ),
        $sa("#sa_verify_otp").off().on("click", {
            btn_class: submit_selector
        }, validateOtp),
    ...
    $sa(".saeditphone").off().on("click", {
        phone_class: phone_selector
    }, saEditNumber);
    var otp_in_popup = sa_otp_settings['otp_in_popup'];
    if ("on" === otp_in_popup) {
            ...
            setTimeout(
                function() {
                    if (modal_style != 'modal-slide') {
                        parentForm.parent().animate({
                            "min-height": currentModel.height()
                        }, 200);
                    }
                    parentForm.hide();
                }, 1000
...
function otpSuceess(o, current_btn, callback) {
    ...
    if ("on" === otp_in_popup) {
        ...
        current_modal.find('.modal-content').css({
            'transform': 'scale(1)',
            'transition': 'all 0.5s'
        });
    }
...
function send_otp(obj, submit_selector, phone_selector, username_selector, password_selector) {
    ...
    if ($sa(obj).parents("form").find(".sa_field_error").length === 0 && verification_allowed) {
        ...
        setTimeout(
            function() {
                $sa(".wc-block-components-checkout-place-order-button").not(".sa-otp-btn-init").addClass("sa-default-btn-hide")
            }, 10
        );
        return false;
    }
...
function validateOtp(event) {
    ...
    sa_validateOTP(
        submit_selector, action_url, data,
        function() {
            $sa("body").off("submit");
...
function initialiseCountrySelector(phoneSelector = '.phone-valid') {
    ...
    var object = $sa(this).saIntellinput({
        hiddenInput: false
    });
...
function saReSubmit(obj) {
    ...
    $sa(".sa-otp-initiated .phone-valid").trigger('keyup');
    $sa('.sa-otp-initiated .sa-otp-btn-init').trigger('click');
}
